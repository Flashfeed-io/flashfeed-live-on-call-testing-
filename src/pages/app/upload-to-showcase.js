...
const app = createApp({
  ...
  get getCheckedIndustries() {
    return this.getIndustries.filter((skill) =>
      store.fields.categories.includes(skill.airtable_id)
    );
  },
  get getCheckedGenreForVideo() {
    return this.getGenreForVideo.filter((skill) =>
      store.fields.categories.includes(skill.airtable_id)
    );
  },
  get getCheckedGenreForPhoto() {
    return this.getGenreForPhoto.filter((skill) =>
      store.fields.categories.includes(skill.airtable_id)
    );
  },
